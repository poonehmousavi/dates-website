...
function normalizeGroup(group) {
  ...
  function audioUrl({ group, subpart, tokenizer, idx }) {
    ...
    const base = import.meta.env.BASE_URL; // Vite's base path (will be "/dates-website/" in your setup)
...
export default function AudioSamples() {
  // Panel state: which top-level and sub-panel is open
          ...
          <div key={acc.main} className="border rounded-xl mb-2">
            ...
            <button
              ...
              className={`w-full text-left p-4 font-semibold text-lg select-none focus:outline-none ${
                openMain === mainIdx
                  ? `bg-${acc.color}-200 text-${acc.color}-900`
                  : `bg-${acc.color}-100 text-${acc.color}-800`
              }`}
              type="button"
